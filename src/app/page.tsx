...
import { Award, Bell, BookOpen, Check, ChevronDown, ClipboardList, Eye, FileCheck2, FileText, GraduationCap, ImagePlus, LayoutDashboard, Menu, MoreHorizontal, Move, Pencil, Plus, Printer, RotateCcw, Save, Search, Settings, SlidersHorizontal, Sparkles, Trash2, UserPlus, Users, X } from "lucide-react";
...
export default function Home() {
  ...
  const [institution, setInstitution] = useState<InstitutionSettings>(defaultSettings);
  const [settingsForm, setSettingsForm] = useState<InstitutionSettings>(defaultSettings);
  ...
  const selectedIdRef = useRef(demoStudents[0].id);
...
function Dashboard(props: { students: Student[]; current: Student; subjects: Subject[]; scores: number[]; average: number; passed: boolean; issuedCount: number; passedCount: number; pendingCount: number; progress: number; search: string; setSearch: (v: string) => void; choose: (s: Student) => void; navigate: (v: string) => void }) {
  return <><section className="metrics-grid"><Metric icon={<Users />} label="Total Santri" value={String(props.students.length)} note="terdaftar di Firestore" tone="yellow" /><Metric icon={<FileCheck2 />} label="Ijazah Terbit" value={String(props.issuedCount)} note="siap dicetak" tone="green" /><Metric icon={<Check />} label="Santri Lulus" value={String(props.passedCount)} note="nilai akhir ≥ 70" tone="blue" /><Metric icon={<ClipboardList />} label="Menunggu Validasi" value={String(props.pendingCount)} note="perlu diterbitkan" tone="orange" /></section><section className="workflow-strip"><div className="workflow-title"><div className="step-badge"><Sparkles size={16} /></div><div><strong>Alur penerbitan ijazah</strong><span>{props.pendingCount} ijazah masih menunggu validasi</span></div></div><div className="progress-line"><div className="progress-done" style={{ width: `${props.progress}%` }} /></div><span className="progress-label">{props.progress}% selesai</span><button className="text-button" onClick={() => props.navigate("Ijazah")}>Lihat semua →</button></section><div className="content-grid"><section className="panel students-panel"><PanelHeader title="Santri terbaru" subtitle="Data realtime tahun ajaran ini." action={<button className="ghost-button" onClick={() => props.navigate("Data Santri")}>Lihat semua →</button>} /><SearchBox value={props.search} onChange={props.setSearch} /><StudentTable students={props.students.filter((s) => s.name.toLowerCase().includes(props.search.toLowerCase())).slice(0, 5)} selectedId={props.current.id} onSelect={props.choose} /></section><GradePanel current={props.current} subjects={props.subjects} scores={props.scores} setScores={() => undefined} average={props.average} passed={props.passed} readOnly onSave={() => props.navigate("E-Raport")} /></div></>;
}
...
function GradePanel({ current, subjects, scores, setScores, average, passed, onSave, readOnly = false }: { current: Student; subjects: Subject[]; scores: number[]; setScores: (v: number[]) => void; average: number; passed: boolean; onSave: () => void; readOnly?: boolean }) {
  return <section className="panel grade-panel"><PanelHeader title="Input nilai" subtitle={`${current.name} · ${current.id}`} /><div className="grade-summary"><div><span>Nilai rata-rata</span><strong>{average}</strong><small className={passed ? "good" : "bad"}>{passed ? "Lulus" : "Belum lulus"} · {scoreWord(average)}</small></div><div className="ring" style={{ "--score": `${average * 3.6}deg` } as React.CSSProperties}><div><strong>{numberToArabic(average)}</strong><span>/ ١٠٠</span></div></div></div><div className="grade-list">{subjects.filter((subject) => subject.active).map((subject) => <label className="grade-row" key={subject.id}><span>{subject.name}<small>{subject.arabicName}</small></span><input aria-label={`Nilai ${subject.name}`} disabled={readOnly} type="number" min="0" max="100" value={scores[subject.order] ?? 0} onChange={(e) => setScores(scores.map((score, index) => index === subject.order ? Math.max(0, Math.min(100, Number(e.target.value))) : score))} /><span className="arabic-number">{numberToArabic(scores[subject.order] ?? 0)}</span></label>)}</div><button className="save-button" onClick={onSave}>{readOnly ? <><Pencil size={16} />Buka input nilai</> : <><Check size={16} />Simpan nilai</>}</button></section>;
}
...
function TemplateDesigner({ template, setTemplate, student, scores, subjects, institution, onSave, notify }: { template: CertificateTemplate; setTemplate: (value: CertificateTemplate) => void; student: Student; scores: number[]; subjects: Subject[]; institution: InstitutionSettings; onSave: () => void; notify: (message: string) => void }) {
  ...
  const visibleIds = (Object.keys(templateLabels) as TemplateElementId[]).filter((id) => page === 1 ? id.startsWith("cover") : id.startsWith("transcript"));
...
function CertificatePages({ student, scores, subjects, institution, template, page, editable, selected, onSelect, onMove }: { student: Student; scores: number[]; subjects: Subject[]; institution: InstitutionSettings; template: CertificateTemplate; page?: 1 | 2; editable?: boolean; selected?: TemplateElementId; onSelect?: (id: TemplateElementId) => void; onMove?: (id: TemplateElementId, x: number, y: number) => void }) {
  ...
  const watermark = (id: "coverWatermark" | "transcriptWatermark") => <>{template.watermarkDataUrl ? <span className="image-layer" style={{ backgroundImage: `url(${template.watermarkDataUrl})`, width: `${template.watermarkSize * 4}px`, height: `${template.watermarkSize * 4}px`, opacity: template.watermarkOpacity }} /> : <div className="default-watermark" style={{ width: `${template.watermarkSize * 4}px`, height: `${template.watermarkSize * 4}px`, opacity: template.watermarkOpacity }}><BookOpen size={74} /><span>{text(id)}</span></div>}</>;
...
function SearchBox({ value, onChange }: { value: string; onChange: (v: string) => void }) { return <div className="toolbar"><div className="search-box"><Search size={16} /><input aria-label="Cari santri" placeholder="Cari nama santri..." value={value} onChange={(e) => onChange(e.target.value)} /></div></div>; }
...
function StudentTable({ students, selectedId, onSelect }: { students: Student[]; selectedId: string; onSelect: (s: Student) => void }) { return <div className="student-table"><div className="table-head"><span>Nama santri</span><span>Jenjang</span><span>Status</span><span>Nilai akhir</span><span /></div>{students.map((student) => <button className={`student-row ${selectedId === student.id ? "selected" : ""}`} key={student.id} onClick={() => onSelect(student)}><StudentIdentity student={student} /><span className="level">{student.level}</span><StatusBadge status={student.status} /><span className="table-score">{student.score}</span><MoreHorizontal size={17} className="muted-icon" /></button>)}</div>; }
function Metric({ icon, label, value, note, tone }: { icon: ReactNode; label: string; value: string; note: string; tone: string }) { return <div className="metric-card"><div className={`metric-icon ${tone}`}>{icon}</div><div className="metric-copy"><span>{label}</span><strong>{value}</strong><small>{note}</small></div></div>; }
